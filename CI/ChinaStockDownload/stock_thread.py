...
class StockDataUpdateThread(threading.Thread):
    def __init__(self, symbols,startday, endday, error_symbols):
        threading.Thread.__init__(self)
        self.t_symbols = symbols
        self.t_errorsymbols = error_symbols
    ...
    def run(self):
        ...
        while not self.t_symbols.empty():
            stock = self.t_symbols.get()
            print str(self), ":", stock
            ...
            try:
                print str(self), ":","urlopen"
                resp = urllib2.urlopen(url, timeout=5)
                print str(self), ":", "url read"
                data = resp.read()
                """no trade in data, only head"""
                if len(data) < 50:
                    continue
                if bFileExisti:
...
#                self.t_symbols.put(stock)
#                print str(self), "lock acquire"
